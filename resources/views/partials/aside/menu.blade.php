...
<div class="hover-scroll-overlay-y my-5 my-lg-5" id="kt_aside_menu_wrapper" data-kt-scroll="true"
    ...
    <div class="menu menu-column menu-title-gray-800 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-500"
        ...
        @php
            $seller_and_childs=['permissions','permissions/create','roles','roles/create','users','users/create'];
            $path=request()->path();
            $pathedit = explode("/",$path);
            if((count($pathedit)==3 && $pathedit[0]=='permissions' && $pathedit[2]=='edit')){
                $seller_and_childs[]=$path;
            }
            if((count($pathedit)==3 && $pathedit[0]=='roles' && $pathedit[2]=='edit')){
                $seller_and_childs[]=$path;
            }
            if((count($pathedit)==3 && $pathedit[0]=='users' && $pathedit[2]=='edit')){
                $seller_and_childs[]=$path;
            }
            if((count($pathedit)==2 && $pathedit[0]=='sellers')){
                $seller_and_childs[]=$path;
            }
            $enable_parent_category=in_array($path,$seller_and_childs)?'hover show':'';
        @endphp
        @canany(['User Management','User','Role','Permission'])
        <div data-kt-menu-trigger="click" class="menu-item menu-accordion {{ $enable_parent_category }}">
